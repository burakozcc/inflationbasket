...
export const InvestmentDetailPage = () => {
    const { id } = useParams();
    const { investments, deleteInvestment, formatPrice, convertPrice } = useAppStore();
    ...
    // Convert History
    const convertedHistory = inv.history.map(p => ({ ...p, value: convertPrice(p.value) }));
        ...
        <ScreenWrapper className="pb-24 border-x border-white/5">
                ...
                <div className="px-4 mb-6 mt-6">
                    <div className="relative overflow-hidden rounded-2xl bg-surface-dark border border-white/5 p-5 shadow-lg">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-bold text-white">Real Return Analysis</h2>
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-4">
                            <StatBox label="Nominal" val="+4.8%" />
                            <StatBox label="Inflation" val="6.2%" />
                            <div className="flex flex-col p-3 rounded-xl bg-background-dark border border-white/5 relative overflow-hidden">
                                ...
                                <span className="text-base font-bold text-loss relative z-10">-1.4%</span>
                            </div>
                        </div>
                        <div className="flex gap-3 items-start">
                            ...
                            <div>
                                <p className="text-sm text-slate-300 leading-relaxed">
                                    <span className="font-bold text-loss">Lagging Inflation.</span> Based on your personal inflation rate, this asset has effectively lost value over the period.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-40 p-4 pt-2 bg-gradient-to-t from-background-dark via-background-dark to-transparent max-w-md mx-auto">
                <div className="flex gap-3">
                    <button className="flex-1 h-12 rounded-xl bg-surface-highlight hover:bg-surface-highlight/80 text-white font-semibold text-base transition-all">Sell</button>
