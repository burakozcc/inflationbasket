...
export const HomePage = () => {
    const { basketItems, currencySymbol, formatPrice, convertPrice } = useAppStore();
    const navigate = useNavigate();
    const [timeRange, setTimeRange] = useState<'1W' | '1M' | '3M' | '1Y'>('3M');

    const totalInflation = basketItems.reduce((acc, item) => acc + item.inflationRate, 0) / (basketItems.length || 1);
    ...
    }, [basketItems, timeRange, convertPrice]);
    ...
    const categoryData = useMemo(() => {
        ...
        // Values here are relative for the Pie Chart, but ideally should be converted if we displayed tooltips with values
        // We'll convert them just to be safe if Recharts uses them for anything absolute
                ...
                <div className="group relative flex w-full flex-col overflow-hidden rounded-xl bg-surface-dark shadow-none ring-1 ring-white/10">
                    <div className="relative z-10 p-6 pb-2">
                        <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-400">Personal Inflation Rate</span>
                            <span className="flex items-center gap-1 rounded-full bg-red-900/30 px-2 py-0.5 text-xs font-bold text-red-400">
                        ...
                        <p className="mt-1 text-sm font-medium text-gray-400">
                            <span className="text-red-400 font-bold">+0.5%</span> vs National Avg
                        </p>
                    </div>
                    ...
                    <div className="flex w-full border-t border-white/5 p-2">
                        {(['1W', '1M', '3M', '1Y'] as const).map((t) => (
                            <button key={t} onClick={() => setTimeRange(t)} className={`flex-1 rounded-lg py-2 text-xs font-semibold transition-all ${timeRange === t ? 'bg-white/10 text-white shadow-sm' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200'}`}>{t}</button>
                        ))}
                    </div>
                </div>
        ...
        <ScreenWrapper className="pb-24">
            ...
            <main className="flex flex-col px-4 pt-4 gap-4">
                <div className="bg-surface-dark rounded-xl p-4 border border-white/5 shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wider">Avg Inflation</p>
    ...
    return (
        <ScreenWrapper className="pb-24">
            ...
            <main className="flex flex-col px-4 pt-6 gap-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-surface-dark p-4 rounded-xl border border-white/5">
                        <p className="text-gray-400 text-xs uppercase font-bold">Inflation</p>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-background-dark border-t border-white/10 z-20">
                <button onClick={() => navigate(`/add?mode=price_entry&relatedId=${item.id}`)} className="w-full bg-primary text-black font-bold h-12 rounded-xl">Update Price</button>
            </div>
        </ScreenWrapper>
    );
};